...
def rich_data_sustituyese_parrafo(num, x, titulo, titulo_titulo, capitulo, capitulo_titulo):
    ...
    m = re.match(r'(?:.*?)Sustitúyese,?(?: en)? el(?: (primer|segundo|tercer|cuarto|quinto))? párrafo(?: (primero|segundo|tercero|cuarto))? del artículo (\d+)[°º]?(?: (:bis|ter|quater|quáter|quinquies|sixties|septies))? de la (Ley|Decreto)(?:.*?) (?:N[°º] )?([\d\.\/]+)(?:.*?) por el siguiente:(.*)', x, re.MULTILINE|re.DOTALL|re.I)
    if m is None: return None
...
def rich_data_197(num, x, titulo, titulo_titulo, capitulo, capitulo_titulo):
    return {
        "fechaDescarga": "29/12/2023, 08:50:32",
        "json_original": {
            "tipoNorma": "",
...
def rich_data_198(num, x, titulo, titulo_titulo, capitulo, capitulo_titulo):
    return {
        "fechaDescarga": "29/12/2023, 08:50:32",
        "json_original": {
            "tipoNorma": "",
...
def rich_data_switch(num, declared_num, x, titulo, titulo_titulo, capitulo, capitulo_titulo):
    ...
    if declared_num == 197:
        return rich_data_197(num, x, titulo, titulo_titulo, capitulo, capitulo_titulo)
    if declared_num == 198:
        return rich_data_198(num, x, titulo, titulo_titulo, capitulo, capitulo_titulo)
    if declared_num == 626:
...
for num, x in enumerate(re.split('[^“]ART[ÍI]CULO', data)):
    ...
    if ESTA is None:
        ...
        texto = {
            ...
            103: 'Ajuste mensual de jubilaciones',
            105: 'Eliminación de jubilación de privilegio para Presidente y Vicepresidente',
            106: 'Eliminación de jubilación de privilegio para Presidente y Vicepresidente',
            107: 'Eliminación de jubilación de privilegio para Presidente y Vicepresidente',
            108: 'Eliminación de jubilación de privilegio para Presidente y Vicepresidente',
            304: 'Derecho a participar en los mercados de biodiésel',
            404: 'Presupuesto de Fondo de Bosques Nativos durante 2024',
            408: 'Derogación de la Ley de Manejo del Fuego',
            449: 'Educación como servicio esencial',
            464: 'Duración de planes de estudio secundario',
            468: 'Máximo de gastos de funcionamiento del INCAA',
            469: 'Lineamiento para la evaluación de proyectos',
            471: 'Coproducciones',
            484: 'Administración del Fondo Nacional de las Artes',
            485: 'Relación del Fondo Nacional de las Artes con el Poder Ejecutivo',
            486: 'Máximo de gastos de funcionamiento del Fondo Nacional de las Artes',
            490: 'Máximo de gastos de funcionamiento de la Comisión Nacional de Bibliotecas Populares',
        }.get(num, texto))
